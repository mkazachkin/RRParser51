...
Public Function ParsXMLFlat051(ByVal tblName As String, ByVal tblKeyName As String, ByVal tblKeyValue As String, ByVal flatNode As Object) As String
    ...
    ' ------------------------
    'Получаем теги
    Dim flatXMLTags(26) As String
        flatXMLTags = GetFlatConfig051 (true)
    'Получаем поля БД
    Dim flatDBFields(26) As String
        flatDBFields = GetFlatConfig051 (false)
        flatDBFields(25) = tblKeyName
    'Инициализируем значения
    Dim flatDBValues(26) As String
    'Получаем типы данных
    Dim flatDBTypes(26) As Boolean
    ...
    While (Not flatChild Is Nothing)
        ...
        If (flatChild.NodeName = flatXMLTags(15)) Then flatDBValues(16) = ParsXMLSubb051(tblName & "_subf", "flat_id", flat_id, cadNum, flatChild)
        ...
        If (flatChild.NodeName = flatXMLTags(20)) Then 
            flatDBValues(20) = parsXMLAddress051(flatChild)
