...
Public Function ParsXMLDocs051(ByVal tblName As String, ByVal tblKeyName As String, ByVal tblKeyValue As String, ByVal cadNum As String, ByVal docsNode As Object) As String
    ...
    Dim docsXMLTags(6) As String
        docsXMLTags(0) = "CodeDocument"
        docsXMLTags(1) = "Name"
        docsXMLTags(2) = "Series"
        docsXMLTags(3) = "Number"
        docsXMLTags(4) = "Date"
        docsXMLTags(5) = "IssueOrgan"
        docsXMLTags(6) = "Desc"

    'Поля в таблице кадастровых стоимостей в БД
    Dim docsDBFields(9) As String
        docsDBFields(0) = "CodeDocument"
        docsDBFields(1) = "Names"
        docsDBFields(2) = "Series"
        docsDBFields(3) = "Numbers"
        docsDBFields(4) = "Dates"
        docsDBFields(5) = "IssueOrgan"
        docsDBFields(6) = "Descr"
        docsDBFields(7) = tblKeyName                        'Идентификатор в таблице объектов, для которой парсится кадастовая стоимость
        docsDBFields(8) = "CadastralNumber"                 'Кадастровый номер объекта, для которого парсится кадастровая стоимость
        docsDBFields(9) = "Reserved"                        'Зарезервированное служебное поле
    ...
    For i = 0 To 8
        insertSQL = insertSQL & docsDBFields(i) & "=" & docsDBValues(i)
    Next i
    insertSQL = insertSQL & " where docs_id = " & docs_id & ";"
